    ...
    while True:
        try:
            ...
            if location is not None:
                ...
                pyautogui.click(center.x, center.y, clicks=clickTimes, interval=0.05, duration=0.1, button=lOrR)
                print(f"找到并点击了 {img}")
                break
        except Exception as e:
            print(f"未找到 {img}，继续寻找...")
        
        if reTry != -1:
            reTry -= 1
            if reTry <= 0:
                print(f"重试次数用尽，跳过 {img}")
                break
...
def mainWork(sheet1):
    ...
    while i < sheet1.nrows:
        try:
            ...
            print(f"执行第 {i} 行时发生错误: {str(e)}，跳过并继续执行")
            i += 1
